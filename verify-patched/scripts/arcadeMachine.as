package
{
   public class arcadeMachine extends staticInteractObjects
   {
      
      private static var selected:Boolean;
      
      private static var selectedArcade:arcadeMachine;
      
      private static var waitOnce:Boolean;
      
      private static var numPlayers:uint = 0;
      
      private var tempN:uint;
      
      private var screenN:uint;
      ...
      private var categoryN:uint;
      
      private var oMouseY:Number;
      
      private var selectNum:Boolean;
      
      private var menuItems:Vector.<menuItem> = new Vector.<menuItem>();
      
      private var playerNumItems:Vector.<playerNumSelect> = new Vector.<playerNumSelect>();
      
      private var loadsArray:Array = ["Races","Arena","Level","Custom"];
      
      private var namesArray:Array = ["Race","Arena","Extra","Custom"];
      
      private var dirArray:Array = ["Arcade","Arcade","Extra","Custom"];
      
      private var listArray:Array = [5,1,4,5];
      
      public function arcadeMachine(p:*)
      {
         isWide = 60;
         ...
         if(!selected)
         {
            if(Math.abs(ex - x) < isWide)
            {
               if(char.UpIsDown())
               {
                  ...
                  if(Boolean(rootHUD.HUD.isReallyMobile) && this.which == "arcadeRemix")
                  {
      ...
      private function buildScreen() : void
      {
         ...
         for(i = 0; i < this.items; i++)
         {
            ...
            if(isNaN(Main.localScores[this.loadsArray[this.categoryN] + i + "_Time"]))
            {
               delete Main.localScores[this.loadsArray[this.categoryN] + i + "_Time"];
            }
            if(!(Main.localScores[this.loadsArray[this.categoryN] + i + "_Time"] == undefined || Main.localScores[this.loadsArray[this.categoryN] + i + "_Time"] == 0))
            {
               this.tempTime = Main.localScores[this.loadsArray[this.categoryN] + i + "_Time"];
               this.tempTime *= 100;
               this.tempTime = Math.round(this.tempTime);
               this.tempTime /= 100;
               this.tempString = String(this.tempTime);
      ...
      private function killScreen() : void
      {
         ...
         if(this.selectNum)
         {
            for(i = 0; i < this.playerNumItems.length; i++)
            {
               this.playerNumItems[i].parent.removeChild(this.playerNumItems[i]);
            }
            this.playerNumItems = new Vector.<playerNumSelect>();
      ...
      override public function InteractEnterFrame() : Boolean
      {
         ...
         if(this.selectNum)
         {
            ...
            else if(Boolean(this.myChar.AttackIsDown()) || Boolean(this.myChar.DownIsDown()))
            {
               this.stepBack();
            }
            else if(this.myChar.wantRL < -0.5 || Boolean(this.myChar.LeftIsDown()))
            {
            ...
            else if(this.myChar.wantRL > 0.5 || Boolean(this.myChar.RightIsDown()))
            {
