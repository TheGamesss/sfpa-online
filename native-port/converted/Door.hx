...
@:meta(Embed(source="/_assets/assets.swf",symbol="symbol4427"))

class Door extends StaticInteractObjects
{
    ...
    private var myText : Dynamic;
    
    private var disabled : Bool;
    
    private var finishDoor : Bool;
    ...
    private var charsFinished : Array<Char>;
    
    public function new(p : Dynamic)
    {
        var i : String = null;
        var tempString : String = null;
        this.charsFinished = new Array<Char>();
        ...
        {
            Reflect.setField(DoorArray, Std.string(ID), this);
        }
        ...
        else if (Main.LoadIt == "Menus1" && warpLevel.substr(0, 5) == "Level" && as3hx.Compat.parseFloat(warpLevel.substr(5, 1)) > Main.localSettings.W1RProgress)
        {
            gotoAndStop("disabled");
        }
        else if (Main.LoadIt == "Menus3" && warpLevel.substr(0, 5) == "Level")
        {
            if (as3hx.Compat.parseFloat(warpLevel.substr(5, 1)) >= Main.localSettings.W1RProgress)
            {
                gotoAndStop("disabled");
            }
            else
    ...
    override public function hitChar(ex : Float, ey : Float, eRL : Float, eUD : Float, char : Char) : Bool
    {
        if (!this.disabled)
        {
            ...
            else
            {
                ...
                if (char.UpIsDown() || cast(Main.localSettings.oneHanded && char.DownIsDown() && (char.Status == "Duck" || char.Status == "DownSlide"), Bool) && cast(char.char.currentFrame == 2, Bool))
                {
