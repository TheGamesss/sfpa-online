...
class Sounds
{
    ...
    public static var pausedBackgroundPosition : Float;
    
    public static var oldPausedBackgroundPosition : Float;
    
    public static var tempPausedBackgroundPosition : Float;
    
    public static var musicFadeToVolume : Float;
    
    private static var fadeAccel : Float;
    
    private static var musicAfterFade : String;
    
    private static var crossFade : Bool;
    ...
    public static var musicFadeVolume : Float = 0;
    
    private static var oldMusicFadeVolume : Float = 0;
    
    private static var tempTransform : SoundTransform = new SoundTransform();
    
    private static var tempVol : Float = 0;
    
    private static var tempPan : Float = 0;
    
    public static var superSound : String = "nothing";
    
    public static function playSound(sound : Dynamic, ex : Dynamic, vol : Dynamic, rail : Dynamic) : Dynamic
    {
    }
    public function new()
    {
    ...
    public static function startLoadSounds() : Dynamic
    {
        soundNum = -1;
    ...
    public static function playMusic(level : Dynamic, accel : Float = 0.02) : Bool
    {
        return playDirect(getMusic(level), accel);
    }
    
    private static function loadSoundsLocal() : Dynamic
    {
        var i : Int = 0;
        var classReference : Class<Dynamic> = null;
        while (i < soundsArray.length)
        {
            classReference = Type.getClass(Type.resolveClass(soundsArray[i]));
            Reflect.setField(allSounds, Std.string(soundsArray[i]), Type.createInstance(classReference, []));
            if (soundsArray[i].substr(0, 6) == "Impact")
            {
                Reflect.setField(soundsVariance, "Impact", 4);
            }
            else if (soundsArray[i].substr(0, 4) == "Coin")
            {
                Reflect.setField(soundsVariance, "Coin", 5);
            }
            else if (soundsArray[i].substr(0, 4) == "Jump")
            {
                Reflect.setField(soundsVariance, "Jump", 5);
            }
            else if (soundsArray[i].substr(0, 4) == "Land")
            {
                Reflect.setField(soundsVariance, "Land", 3);
            }
            else if (soundsArray[i].substr(0, 5) == "Swosh")
            {
                Reflect.setField(soundsVariance, "Swosh", 4);
            }
            else if (soundsArray[i].substr(0, 8) == "MedSwosh")
            {
                Reflect.setField(soundsVariance, "MedSwosh", 4);
            }
            else if (soundsArray[i].substr(0, 10) == "HeavySwosh")
            {
                Reflect.setField(soundsVariance, "HeavySwosh", 2);
            }
            else if (soundsArray[i].substr(0, 8) == "Footstep")
            {
                Reflect.setField(soundsVariance, "Footstep", 10);
            }
            else if (soundsArray[i].substr(0, 3) == "Hit")
            {
                Reflect.setField(soundsVariance, "Hit", 4);
            }
            else if (soundsArray[i].substr(0, 8) == "BadStomp")
            {
                Reflect.setField(soundsVariance, "BadStomp", 4);
            }
            else if (soundsArray[i].substr(0, 9) == "NinjaJump")
            {
                Reflect.setField(soundsVariance, "NinjaJump", 6);
            }
            else if (soundsArray[i].substr(0, 10) == "NinjaSwipe")
            {
                Reflect.setField(soundsVariance, "NinjaSwipe", 4);
            }
            else if (soundsArray[i].substr(0, 9) == "SpiderDie")
            {
                Reflect.setField(soundsVariance, "SpiderDie", 3);
            }
            else if (soundsArray[i].substr(0, 8) == "NinjaDie")
            {
                Reflect.setField(soundsVariance, "NinjaDie", 3);
            }
            else if (soundsArray[i].substr(0, 8) == "MouseDie")
            {
                Reflect.setField(soundsVariance, "MouseDie", 3);
            }
            else if (soundsArray[i].substr(0, 10) == "MouseShoot")
            {
                Reflect.setField(soundsVariance, "MouseShoot", 4);
            }
            else if (soundsArray[i].substr(0, 7) == "InkBoom")
            {
                Reflect.setField(soundsVariance, "InkBoom", 3);
            }
            else if (soundsArray[i].substr(0, 10) == "InkExplode")
            {
                Reflect.setField(soundsVariance, "InkExplode", 4);
            }
            else if (soundsArray[i].substr(0, 12) == "PillarImpact")
            {
                Reflect.setField(soundsVariance, "PillarImpact", 4);
            }
            else if (soundsArray[i].substr(0, 8) == "HangLand")
            {
                Reflect.setField(soundsVariance, "HangLand", 2);
            }
            else if (soundsArray[i].substr(0, 8) == "HangStep")
            {
                Reflect.setField(soundsVariance, "HangStep", 10);
            }
            else if (soundsArray[i].substr(0, 5) == "Twirl")
            {
                Reflect.setField(soundsVariance, "Twirl", 6);
            }
            else if (soundsArray[i].substr(0, 13) == "InkBoardStart")
            {
                Reflect.setField(soundsVariance, "InkBoardStart", 2);
            }
            else if (soundsArray[i].substr(0, 9) == "PenCharge")
            {
                Reflect.setField(soundsVariance, "PenCharge", 3);
            }
            else if (soundsArray[i].substr(0, 10) == "BreakBlock")
            {
                Reflect.setField(soundsVariance, "BreakBlock", 4);
            }
            else if (soundsArray[i].substr(0, 7) == "InkShot")
            {
                Reflect.setField(soundsVariance, "InkShot", 10);
            }
            else if (soundsArray[i].substr(0, 8) == "InkBurst")
            {
                Reflect.setField(soundsVariance, "InkBurst", 4);
            }
            else if (soundsArray[i].substr(0, 8) == "InkSplat")
            {
                Reflect.setField(soundsVariance, "InkSplat", 4);
            }
            else if (soundsArray[i].substr(0, 7) == "InkSpit")
            {
                Reflect.setField(soundsVariance, "InkSpit", 3);
            }
            else if (soundsArray[i].substr(0, 7) == "InkHurt")
            {
                Reflect.setField(soundsVariance, "InkHurt", 3);
            }
            else if (soundsArray[i].substr(0, 7) == "InkLand")
            {
                Reflect.setField(soundsVariance, "InkLand", 6);
            }
            else if (soundsArray[i].substr(0, 7) == "InkJump")
            {
                Reflect.setField(soundsVariance, "InkJump", 6);
            }
            else if (soundsArray[i].substr(0, 9) == "FallInInk")
            {
                Reflect.setField(soundsVariance, "FallInInk", 2);
            }
            else
            {
                Reflect.setField(soundsVariance, Std.string(soundsArray[i]), 0);
            }
            i++;
        }
    }
    
    public static function playDirect(music : Dynamic, accel : Float = 0.02, limit : Float = 1, after : String = "loop", startAt : Int = 0) : Bool
    {
        var classReference : Class<Dynamic> = null;
        if (music != musicPlaying)
        {
            ...
            if (music != "nothing")
            {
                classReference = Type.getClass(Type.resolveClass(music));
                BackgroundMusic = Type.createInstance(classReference, []);
    ...
    {
        var classReference : Class<Dynamic> = Type.getClass(Type.resolveClass(music));
        tempBackgroundMusic = Type.createInstance(classReference, []);
    ...
    public static function tempForcePosition(e : Int) : Void
    {
    ...
    private static function cosCurve(e : Float) : Float
    {
    ...
    public static function fadeOutMusic(music : String = "nothing", accel : Float = 0.02, fadeType : String = "crossFade", limit : Float = 1, after : String = "loop") : Bool
    {
    ...
    public static function getMusic(loadit : Dynamic, door : Int = 0) : Dynamic
    {
        ...
        if (Main.DirIt == "World 4")
        {
            ...
            if (loadit == "bonus")
            {
                return "Challenge0" + (as3hx.Compat.parseInt(Math.random() * 3) + 1);
            }
    ...
    public static function updateMusic(vol : Dynamic) : Dynamic
    {
        BackgroundChannel.soundTransform = new SoundTransform(vol * musicVol);
    }
    
    public static function updateOldMusic(vol : Dynamic) : Dynamic
    {
        oldBackgroundChannel.soundTransform = new SoundTransform(vol * musicVol);
    }
    
    public static function setVolume(vol : Dynamic) : Dynamic
    {
        if (vol == 0 && sfxVol > 0)
        {
            muteAll();
    ...
    public static function setMusic(vol : Dynamic) : Dynamic
    {
    ...
    private static function resetPosition(e : Event) : Dynamic
    {
        var classReference : Class<Dynamic> = null;
        var old : Bool = e.currentTarget == oldBackgroundChannel;
        e.currentTarget.removeEventListener(Event.SOUND_COMPLETE, resetPosition);
        if (musicPlayAfter != "loop")
        {
            classReference = Type.getClass(Type.resolveClass(musicPlayAfter));
            BackgroundMusic = Type.createInstance(classReference, []);
            musicPlayAfter = "loop";
        }
        if (old)
    ...
    public static function muteButton() : Dynamic
    {
    ...
    public static function muteAll() : Dynamic
    {
        playSound = function(sound : Dynamic, ex : Dynamic, vol : Dynamic, rail : Dynamic) : Dynamic
                {
                };
    }
    
    public static function unMuteAll() : Dynamic
    {
        playSound = reallyPlaySound;
    }
    
    public static function pauseMusic() : Dynamic
    {
        if (musicVol > 0)
        {
            if (BackgroundMusic != null)
    ...
    public static function resumeMusic() : Dynamic
    {
        if (musicVol > 0)
        {
            if (BackgroundMusic != null)
    ...
    private static function soundError(e : Event) : Dynamic
    {
        sound.addEventListener(Event.COMPLETE, soundLoaded);
        sound.addEventListener(IOErrorEvent.IO_ERROR, soundError);
        if (soundNum == -1)
    ...
    private static function randNum(e : Dynamic) : Dynamic
    {
        return Math.floor(Math.random() * e);
    }
    
    public static function reallyPlaySound(sound : String, ex : Float, vol : Float = 1, rail : Int = 0) : Dynamic
    {
        if (Reflect.field(soundsVariance, sound) == 0)
        {
            Reflect.field(allSounds, sound).play(0, 1, getTransform(ex, vol, rail));
        }
        else
        {
            Reflect.field(allSounds, Std.string(sound + "_" + randNum(Reflect.field(soundsVariance, sound)))).play(0, 1, getTransform(ex, vol, rail));
        }
    }
    
    public static function playSoundSimple(sound : String, vol : Float = 1) : Void
    {
        Reflect.field(allSounds, sound).play(0, 1, new SoundTransform(sfxVol * vol));
    }
    
    public static function playSoundThenLoop(sound : String, source : APlat, vol : Float = 1) : Void
    {
        var channel : SoundChannel = Reflect.field(allSounds, sound).play(0, 1, new SoundTransform(sfxVol * vol));
        superSource = source;
        channel.addEventListener(Event.SOUND_COMPLETE, playLoopAfter);
    }
    
    private static function playLoopAfter(e : Event) : Dynamic
    {
        superSource.startLoop();
    }
    
    public static function playSoundContinuous(sound : Dynamic, ex : Dynamic, vol : Dynamic, rail : Dynamic) : Dynamic
    {
        var channel : SoundChannel = Reflect.field(allSounds, Std.string(sound)).play(0, 1000);
        if (channel != null)
        {
    ...
    public static function updateSound(channel : SoundChannel, ex : Float, vol : Float, rail : Int) : Dynamic
    {
        channel.soundTransform = getTransform(ex, vol, rail);
    }
    
    private static function getTransform(ex : Float, vol : Float, rail : Int) : SoundTransform
    {
    ...
    public static function stopSound(channel : Dynamic) : Dynamic
    {
        channel.stop();
    }
}
