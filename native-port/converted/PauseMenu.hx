...
@:meta(Embed(source="/_assets/assets.swf",symbol="symbol1216"))
...
class PauseMenu extends MovieClip
{
    ...
    public static var gamepadmenu : Dynamic;
    
    public static var bubble : Dynamic;
    ...
    private var currentKnob : Dynamic;
    
    public var IconArray : Array<Dynamic>;
    
    private var pantsIconArray : Array<Dynamic> = [];
    
    private var hatIconArray : Array<Dynamic> = [];
    
    private var patternIconArray : Array<Dynamic> = [];
    
    private var colorIconArray : Array<Dynamic> = [];
    
    private var hasPantsArray : Array<Dynamic> = [];
    
    private var hasHatsArray : Array<Dynamic> = [];
    
    private var hasPatternsArray : Array<Dynamic> = [];
    
    private var hasHatsN : Int;
    
    private var hasPantsN : Int;
    
    private var hasPatternsN : Int;
    
    private var pantsScrollRL : Float = 0;
    
    private var pantsScrollX : Float = 0;
    
    private var oPantsScrollX : Float = 0;
    
    private var hatScrollRL : Float = 0;
    
    private var hatScrollX : Float = 0;
    
    private var oHatScrollX : Float = 0;
    ...
    public var scrollPantsOffset : Float;
    
    public var scrollHatsOffset : Float;
    
    public var forChar : Char;
    
    private var iconPixels : Int;
    
    private var iconRatio : Float;
    
    private var shine : Bool;
    
    private var menuKeyThresh : Float;
    
    public var selectorPos : Dynamic = { };
    
    public var selectorCatN : Int = 0;
    
    public var selectorCat : String;
    
    public var selectorCatArray : Array<String>;
    
    public var canClick : Bool;
    
    public var mouseDownX : Int;
    
    private var keySwitch : Bool;
    ...
    public static function showPause(n : Dynamic) : Dynamic
    {
        if (Main.debug)
        {
    ...
    public static function giveColors() : Dynamic
    {
    ...
    private static function addColors() : Dynamic
    {
        bubble = null;
    ...
    private function setupIcons() : Dynamic
    {
        ...
        this.IconArray = [];
        this.selectorCatArray = new Array<String>();
        ...
        if (this.hasHatsArray.indexOf(true) > -1)
        {
            ...
            for (i in 0...this.hasHatsArray.length + 1)
            {
                if (this.forChar.hatN == i)
                {
                    this.selectorPos.hat = this.IconArray.length;
                }
                if (this.hatIconArray[i] != null)
                {
                ...
                {
                    icon = new PauseIconCache(900, 140, this.hasHatsN, 140, this.iconRatio, this.hasHatsN * 2, "hat", i, this.forChar.hatN == i, -1, i == 0 || cast(this.hasHatsArray[i - 1], Bool));
        ...
        for (i in 0...this.hasPantsArray.length + 4)
        {
            ...
            {
                icon = new PauseIconCache(900, 220, this.hasPantsN, 220, this.iconRatio, this.hasPantsN * 2 + 2, "pants", i, this.forChar.pantsN == i, -1, i < 4 || cast(this.hasPantsArray[i - 4], Bool));
        ...
        if (this.hasPatternsArray.indexOf(true) > -1)
        {
            ...
            for (i in 1...this.hasPatternsArray.length + 1)
            {
                if (this.forChar.patternN == i)
                {
                    this.selectorPos.pattern = this.IconArray.length;
                }
                if (this.patternIconArray[i] != null)
                {
    ...
    public function displayColors() : Dynamic
    {
        PauseIconCache.stamp.gotoAndStop(1);
        ...
        for (i in 0...this.hasPantsArray.length + 4)
        {
            ...
            {
                if (i < 4 || cast(this.hasPantsArray[i - 4], Bool))
                {
    ...
    private function findRatio(array : Dynamic) : Void
    {
        this.iconRatio = 0.8;
    }
    ...
    public function PauseEnterFrame() : Void
    {
        ...
        if (cast(this.keySwitch, Bool) && this.selectorCat != "Map")
        {
        ...
        var temp : Float = -80 * (this.hasPantsN + this.hasPatternsN) * 0.8 + 720;
        ...
        for (i in 0...this.IconArray.length)
        {
    ...
    public function setScrollPantsOffset(ex : Float) : Void
    {
        this.mouseDownX = ex;
    ...
    public function scrollPants(ex : Float) : Void
    {
    ...
    public function setScrollHatsOffset(ex : Float) : Void
    {
        this.mouseDownX = ex;
    ...
    public function scrollHats(ex : Float) : Void
    {
    ...
    private function menuKeys(ex : Float, ey : Float) : Void
    {
        if (Math.abs(ey) > 0.3)
        {
    ...
    public function clickSomething(e : Dynamic, keys : Bool = false) : Bool
    {
        var o : Int = 0;
        ...
        {
            o = as3hx.Compat.parseInt(this[e.type + "N"]);
            if (e.type == "pattern" && this.pantsN > -1)
            {
                Reflect.field(this, "pantsIconArray")[this.pantsN].ratio *= 1.25;
                Reflect.field(this, "pantsIconArray")[this.pantsN].drawOutline(1);
                Reflect.setField(this, "pantsIconArray", false)[this.pantsN].selected;
                this.pantsN = -1;
            }
            else if (e.type == "pants" && this.patternN > 0)
            {
                Reflect.field(this, "patternIconArray")[this.patternN].ratio *= 1.25;
                Reflect.field(this, "patternIconArray")[this.patternN].drawOutline(1);
                Reflect.setField(this, "patternIconArray", false)[this.patternN].selected;
                this.patternN = 0;
            }
            else if (o > -1)
            {
    ...
    private function redrawPatterns() : Void
    {
        if (this.hasPatternsArray.indexOf(true) > -1)
        {
            ...
            for (i in 0...this.hasPatternsArray.length + 1)
            {
                if (this.patternIconArray[i] != null)
                {
    ...
    private function showCombosList(e : MouseEvent) : Dynamic
    {
        Main.parse_orderAScore("maxCombo", true, 15, function(e : Dynamic) : Dynamic
                {
                    var i : Int = 0;
                    var n : Int = 0;
                    Text.text = "\n\n";
                    ...
                    {
                        Text.appendText(Reflect.field(Reflect.field(e, Std.string(i)), Std.string(0)).slice(0, Reflect.field(Reflect.field(e, Std.string(i)), Std.string(0)).length - 3));
                        ...
                        Text.appendText(Reflect.field(Reflect.field(e, Std.string(i)), Std.string(1)) + "\n");
                    }
                });
    }
    
    private function showTimesList(e : MouseEvent) : Dynamic
    {
        Main.stageRoot.MiniclipAPI.services.showHighscores();
    }
    
    private function hideTheUI(e : MouseEvent) : Dynamic
    {
        visible = false;
    ...
    private function volumeKnobStart(e : MouseEvent) : Dynamic
    {
    ...
    private function volumeKnobDrag(e : MouseEvent) : Dynamic
    {
    ...
    private function volumeKnobEnd(e : MouseEvent) : Dynamic
    {
    ...
    private function selectHat(event : MouseEvent) : Dynamic
    {
        this.forChar.changeHat(as3hx.Compat.parseInt(event.target.parent.name.substring(3)));
        Main.unPauseGame();
    }
    
    private function selectPants(e : MouseEvent) : Dynamic
    {
        this.forChar.changePants(as3hx.Compat.parseInt(e.currentTarget.name.substring(5)));
        Main.unPauseGame();
    }
    
    private function selectColor(e : MouseEvent) : Dynamic
    {
        this.forChar.changePatternColor(as3hx.Compat.parseInt(e.currentTarget.name.substring(5)));
        Main.unPauseGame();
    }
    
    private function selectPattern(e : MouseEvent) : Dynamic
    {
        this.forChar.changePattern(as3hx.Compat.parseInt(e.currentTarget.name.substring(7)));
        Main.unPauseGame();
    }
    
    @:allow()
    private function __setProp_timesList_PauseMenu_buttons_0() : Dynamic
    {
        try
        {
    ...
    @:allow()
    private function __setProp_combosList_PauseMenu_buttons_0() : Dynamic
    {
        try
        {
    ...
    @:allow()
    private function __setProp_hideUI_PauseMenu_buttons_0() : Dynamic
    {
        try
        {
    ...
    @:allow()
    private function __setProp_goFullscreen_PauseMenu_buttons_0() : Dynamic
    {
        try
        {
    ...
    @:allow()
    private function __setProp_quitToMenu_PauseMenu_buttons_0() : Dynamic
    {
        try
        {
    ...
    @:allow()
    private function __setProp_setupGamepadButton_PauseMenu_buttons_0() : Dynamic
    {
        if (this.__setPropDict[this.setupGamepadButton] == null || as3hx.Compat.parseInt(this.__setPropDict[this.setupGamepadButton]) != 1)
        {
    ...
    @:allow()
    private function __setProp_debugSelect_PauseMenu_buttons_0() : Dynamic
    {
        if (this.__setPropDict[this.debugSelect] == null || as3hx.Compat.parseInt(this.__setPropDict[this.debugSelect]) != 1)
        {
    ...
    @:allow()
    private function __setProp_quitButton_PauseMenu_buttons_0() : Dynamic
    {
        if (this.__setPropDict[this.quitButton] == null || as3hx.Compat.parseInt(this.__setPropDict[this.quitButton]) != 1)
        {
    ...
    @:allow()
    private function __setProp_creditsButton_PauseMenu_buttons_0() : Dynamic
    {
        if (this.__setPropDict[this.creditsButton] == null || as3hx.Compat.parseInt(this.__setPropDict[this.creditsButton]) != 1)
        {
    ...
    @:allow()
    private function __setProp_languageButton_PauseMenu_buttons_0() : Dynamic
    {
        if (this.__setPropDict[this.languageButton] == null || as3hx.Compat.parseInt(this.__setPropDict[this.languageButton]) != 1)
        {
    ...
    @:allow()
    private function __setProp_setupGamepadButton_PauseMenu_buttons_2() : Dynamic
    {
        if (this.__setPropDict[this.setupGamepadButton] == null || as3hx.Compat.parseInt(this.__setPropDict[this.setupGamepadButton]) != 3)
        {
    ...
    @:allow()
    private function __setProp_debugSelect_PauseMenu_buttons_2() : Dynamic
    {
        if (this.__setPropDict[this.debugSelect] == null || as3hx.Compat.parseInt(this.__setPropDict[this.debugSelect]) != 3)
        {
    ...
    @:allow()
    private function __setProp_quitButton_PauseMenu_buttons_2() : Dynamic
    {
        if (this.__setPropDict[this.quitButton] == null || as3hx.Compat.parseInt(this.__setPropDict[this.quitButton]) != 3)
        {
    ...
    @:allow()
    private function __setProp_creditsButton_PauseMenu_buttons_2() : Dynamic
    {
        if (this.__setPropDict[this.creditsButton] == null || as3hx.Compat.parseInt(this.__setPropDict[this.creditsButton]) != 3)
        {
    ...
    @:allow()
    private function __setProp_languageButton_PauseMenu_buttons_2() : Dynamic
    {
        if (this.__setPropDict[this.languageButton] == null || as3hx.Compat.parseInt(this.__setPropDict[this.languageButton]) != 3)
        {
    ...
    @:allow()
    private function frame3() : Dynamic
    {
