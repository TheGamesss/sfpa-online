package
{
   ...
   [Embed(source="/_assets/assets.swf", symbol="symbol5681")]
   public class BossPencil extends Baddies
   {
      ...
      private function pencilHitChar(ex:*, ey:*, eRL:*, eUD:*, ax:*, ay:*, char:*) : *
      {
         distRL = ex - x;
         distUD = ey - y;
         if(stunN > 0)
      ...
      private function hitBaddie(ex:*, ey:*, eRL:*, eUD:*, baddie:*) : *
      {
         distRL = ex - x;
         distUD = ey - y;
      ...
      private function checkHitBaddies() : *
      {
         var AggressorArrayLength:int = int(AggressorArray.length);
         for(var n:int = 0; n < AggressorArrayLength; n++)
         {
            if(AggressorArray[n].downTime == 0)
            {
               if(Math.abs(x - AggressorArray[n].x) < isTall + AggressorArray[n].isWide && Math.abs(y - AggressorArray[n].y) < isWide + AggressorArray[n].isTall)
               {
                  this.hitBaddie(AggressorArray[n].x,AggressorArray[n].y,AggressorArray[n].moveRL,AggressorArray[n].moveUD,AggressorArray[n]);
               }
            }
         }
      }
      
      private function PencilGetAttacked(ex:Number, ey:Number, angle:Number, char:collision, hitMove:String, hitPower:Number, pow:Number = 1) : Boolean
      {
      ...
      private function PoundEnterFrame() : *
      {
         ...
         if(this.step != 0 && this.step != 4)
         {
            if(moveRL * x > 0)
            {
               if(wallsHitTest(x + UDx(isTall),y + UDy(isTall)))
               {
      ...
      private function DrawEnterFrame() : *
      {
         var spider:Object = null;
         ...
         switch(this.step)
         {
            ...
            case 3:
               wallAngle += 0.7;
               x = wallX + UDx(-25 + Math.random() * 10);
               y = wallY + UDy(-25 + Math.random() * 10);
               if(wallAngle >= 18.84)
               {
                  wallAngle = 0;
                  this.b = 30;
      ...
      private function SpinEnterFrame() : *
      {
         switch(this.step)
         {
            case 0:
      ...
      private function WeakEnterFrame() : *
      {
         switch(this.step)
         {
            case 0:
      ...
      private function DeadEnterFrame() : *
      {
         switch(this.step)
         {
            case 0:
